...
#' @param colname Name of the column containing human gene synonyms to be converted to HGNC approved gene names
#' @param drop If it is TRUE, it will delete row or rows which there is no match between human gene synonyms and HGNC approved gene names
#'
...
hgncConverter<-function(genelist, colname, drop = FALSE){
  ...
  n_occur <- data.frame(table(human_hgnc$Gene_synonyms))
  bg5<-human_hgnc[human_hgnc$Gene_synonyms %fin% n_occur$Var1[n_occur$Freq > 1],]

  bg5<-bg5[which(bg5$Gene_synonyms %fin% genelist[[colname]][which(!(genelist[[colname]] %fin% human_hgnc$Gene_name))]),]
  ...
  for (i in 1:length(genelist[[colname]])){
    if (!(genelist[[colname]][i] %fin% human_hgnc$Gene_name_temp) && length(human_hgnc$Gene_name_temp[which(human_hgnc$Gene_synonyms %fin% genelist[[colname]][i])]) == 1){
      genelist[[colname]][i]<-human_hgnc$Gene_name_temp[which(human_hgnc$Gene_synonyms %fin% genelist[[colname]][i])]
    }
